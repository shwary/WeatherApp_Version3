...
public class SuggestionCalculation {
    ...
    public SuggestionCalculation() {
        allClothes.add("T-Shirt"); // > 60              1
        allClothes.add("Long Sleeve Shirt"); // <= 60   2
        allClothes.add("Shorts"); // >60                3
        allClothes.add("Pants"); // <= 60               4
        allClothes.add("Umbrella"); //Raining / Misty   5
        allClothes.add("Sunglasses"); //Sunny           6
        allClothes.add("Jacket"); //Snowing, <= 40      7
        allClothes.add("Sweatshirt"); //<= 50           8
        allClothes.add("Snow Cap"); //Snowing           9
        allClothes.add("Hat"); //Sunny                  10 As in "baseball cap"
        allClothes.add("Gloves"); // Snowing            11
        allClothes.add("Sweater"); //<=40               12
    }
    ...
    public String[] suggestion(int temp_int, String windSpeed, String weatherDescription){
        ...
        if(weatherDescription.equals("shower rain") || weatherDescription.equals("rain") || weatherDescription.equals("mist")){
            suggestion_list.add(allClothes.get(5)); //Adds Umbrella
            suggestionSize++;
        }

        if(weatherDescription.equals("clear skies")){
            suggestion_list.add(allClothes.get(6)); //Adds Sunglasses
            suggestion_list.add(allClothes.get(10)); //Adds Hat
            suggestionSize+= 2;
        }

        if(weatherDescription.equals("snow")){
            suggestion_list.add(allClothes.get(9)); //Adds Snow Cap
            suggestion_list.add(allClothes.get(11));//Adds Gloves
            suggestion_list.add(allClothes.get(7));//Adds Jacket
            suggestionSize+= 3;
        }

        if( !(weatherDescription.equals("snow")) && temp_int <= 40){
            suggestion_list.add(allClothes.get(7));//Adds Jacket if not snowing but still under 40
            suggestionSize++;
        }

        if(temp_int > 40 && temp_int <= 55){
            suggestion_list.add(allClothes.get(8));//Adds Sweatshirt
            suggestionSize++;
        }
        ...
        if(temp_int > 60){
            ...
            suggestion_list.add(allClothes.get(2)); //Adds Long Sleeve Shirt if under or equal to 60 and greater than 40
            suggestionSize++;
        }
        else{
        ...
        if(temp_int > 70){
            ...
            suggestion_list.add(allClothes.get(4)); //Adds pants if under or equal to 70
            suggestionSize++;
        }
